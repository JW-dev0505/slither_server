...
public class GameState {
  ...
  private final int ORB_GENERATION_TIME_INTERVAL = 30; // time interval at which new orbs are generated
  ...
  public void createNewSnake(User thisUser, WebSocket webSocket, Set<WebSocket> gameStateSockets, SlitherServer server) {
    ...
    for (int i=0; i < 20; i++) {
      ...
      // Position position = new Position(Math.random() % 4000 - 2000, Math.random() % 4000 - 2000 + 5 * i);
  ...
  public boolean outbandCollisionCheck(Position latestHeadPosition)
  {
    if( latestHeadPosition.x() - this.SNAKE_CIRCLE_RADIUS <= -3000 ||
        latestHeadPosition.x() + this.SNAKE_CIRCLE_RADIUS >= 3000 ||
        latestHeadPosition.y() - this.SNAKE_CIRCLE_RADIUS <= -3000 ||
        latestHeadPosition.y() + this.SNAKE_CIRCLE_RADIUS >= 3000
      ) 
      return true;
    return false;
  ...
  public boolean checkSnakeCollision(User thisUser, Position latestHeadPosition, WebSocket webSocket, Set<WebSocket> gameStateSockets, SlitherServer server)
  {
  ...
  public void collisionCheck(User thisUser, Position latestHeadPosition, WebSocket webSocket, Set<WebSocket> gameStateSockets, SlitherServer server) {
    ...
    CompletableFuture.supplyAsync(() -> outbandCollisionCheck(latestHeadPosition)).thenAccept(
      result -> {
        if(result)
        {
    ...
    CompletableFuture.supplyAsync(() -> checkSnakeCollision(thisUser, latestHeadPosition, webSocket, gameStateSockets, server)).thenAccept(
      result -> {
        if(result) {
          Message userDiedMessage = new Message(MessageType.YOU_DIED, new HashMap<>());
          String jsonMessage = server.serialize(userDiedMessage);
